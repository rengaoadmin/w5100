...
if ($_GET['data'] && ($_GET['token'] == "YourTokenWithWechat")) {//可以改token,这相当于密码，在Arduino端改成相应的值即可
	$con = mysql_connect('YourSqlServer:SqlPort','YourUserNmae','YourPassword'); 
	$data = $_GET['data'];
	mysql_select_db("YourDatabaseName", $con);//要改成相应的数据库名
